...
def user_profile():
    conn = get_db_connection()
    users = conn.execute("SELECT * FROM users WHERE id_user = ?", (session['id_user'],)).fetchone()
    conn.close()
...
def user_ruang():
    ...
    user = conn.execute("SELECT * FROM users WHERE id_user = ?", (session['id_user'],)).fetchone()
    conn.close()
    return render_template('user_ruang.html', labs=labs, user=user)

#Pinjam Lab
@app.route('/user/user_pinjam_lab', methods=['GET', 'POST'])
def pinjam_lab():

    id_labs= request.args.get('id_lab', type=int)
    conn = get_db_connection()
    lab_list = conn.execute("SELECT * FROM lab WHERE id_lab = ?", (id_labs,)).fetchall()
    user_list = conn.execute("SELECT * FROM users WHERE id_user = ?", (session['id_user'],)).fetchone()
    ...
    if request.method == 'POST':
        ...
        conn.close()
        return redirect(url_for('pinjam_lab'))

    conn.close()
    return render_template('user_pinjam_lab.html', lab_list=lab_list, users = user_list, id_labs=id_labs)

#Riwayat Peminjaman Lab
@app.route('/user/log_lab')
def riwayat_lab():
    conn = get_db_connection()
    pinjam = conn.execute("SELECT j.id_penjadwalan, u.id_kartu, l.nama_lab, j.start_time, j.end_time, j.status FROM penjadwalan j JOIN lab l ON j.id_lab = l.id_lab JOIN users u ON j.id_user = u.id_user WHERE j.id_user = ?", (session['id_user'],)).fetchall() 
    conn.close()
...
def user_barang():
    ...
    user = conn.execute("SELECT * FROM users WHERE id_user = ?", (session['id_user'],)).fetchone()
    conn.close()
    return render_template('user_barang.html', labs=labs, user=user)

@app.route('/user/user_pinjam_barang', methods=['GET', 'POST'])
def pinjam_barang():
    ...
    lab_list = conn.execute("SELECT * FROM lab WHERE id_lab = ?", (id_labs,)).fetchall()
    user_list = conn.execute("SELECT * FROM users WHERE id_user = ?", (session['id_user'],)).fetchone()
    barang_list = conn.execute("SELECT * FROM inventaris WHERE id_lab = ?",  (id_labs,)).fetchall()
    ...
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO peminjaman (id_user, tanggal_pinjam, tanggal_kembali, status) VALUES (?, ?, ?, ?)", 
                    (id_user, tgl_pinjam, tgl_kembali, "pending"))
        id_peminjaman = cursor.lastrowid
        ...
        return redirect(url_for('pinjam_barang'))

    conn.close()
    return render_template('user_pinjam_barang.html', barang_list=barang_list, lab_list=lab_list, users = user_list, id_labs=id_labs)

#Riwayat Peminjaman Barang
@app.route('/user/log_barang')
def riwayat_barang():
    conn = get_db_connection()
    peminjaman = conn.execute("""
        SELECT p.id_peminjaman, i.nama_barang, l.nama_lab, p.tanggal_pinjam, p.tanggal_kembali, p.status
                               FROM peminjaman p JOIN detail_peminjaman d ON p.id_peminjaman=d.id_peminjaman 
                              LEFT JOIN inventaris i ON d.id_barang = i.id_barang 
                              LEFT JOIN lab l ON i.id_lab=l.id_lab
                              WHERE p.id_user = ?                    
    """, (session['id_user'],)).fetchall()
    conn.close()
    return render_template('user_log_barang.html', peminjaman=peminjaman)

@app.route('/user/log')
def user_log():
    conn = get_db_connection()
    log = conn.execute('SELECT * FROM log JOIN lab ON log.id_lab = lab.id_lab JOIN users ON log.id_kartu = users.id_kartu WHERE id_user=?',(session['id_user'],)).fetchall()
    # log = conn.execute('SELECT * FROM log').fetchall()
    conn.close()
    return render_template('user_log.html', log=log)
